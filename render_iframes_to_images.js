...
class GBPIframeProcessor {
    ...
    async readCSV(filePath) {
        return new Promise((resolve, reject) => {
            ...
            stream
                ...
                .on('end', () => {
                    console.log(`Successfully parsed ${results.length} valid rows from CSV`);
                    resolve(results);
                })
                .on('error', (error) => {
    ...
    async processURL(urlData, retryCount = 0) {
        ...
        try {
            ...
            if (!iframeInfo) {
                throw new Error(`Iframe with src containing "${gbpIframeSrc}" not found`);
            }

            console.log(`Found iframe at position x:${iframeInfo.position.x}, y:${iframeInfo.position.y}`);
            ...
            if (retryCount < this.maxRetries) {
                console.warn(`Retry ${retryCount + 1}/${this.maxRetries} for URL: ${url}`);
    ...
    async navigateToPage(page, url) {
        const strategies = [
            { waitUntil: ['load', 'domcontentloaded'], name: 'load+domcontentloaded' },
        ...
        for (const strategy of strategies) {
            try {
                ...
                console.warn(`Navigation strategy ${strategy.name} failed:`, error.message);
                lastError = error;
                await this.sleep(1000);
            }
        }

        throw new Error(`All navigation strategies failed. Last error: ${lastError?.message || 'Unknown error'}`);
    }
    ...
    async takeScreenshot(page, index, url) {
        ...
        try {
            const filename = `screenshot_${index + 1}_${this.sanitizeFilename(url)}.png`;
        ...
        try {
            ...
            for (let i = 0; i < urlData.length; i++) {
                ...
                if (result.success) {
                    ...
                    console.log(`   Scroll Position: x=${result.scrollPosition.x}, y=${result.scrollPosition.y}`);
            ...
            console.log(`Success Rate: ${((summary.successful / summary.totalProcessed) * 100).toFixed(1)}%`);
...
async function InitializeGBPIframeProcessor(fileToProcess) {
    ...
    try {
        ...
        const processor = new GBPIframeProcessor({
        ...
        const results = await processor.process(csvFilePath);
        
        console.log('\nProcessing completed!');
        console.log('Check the screenshots folder for captured images and processing report.');
